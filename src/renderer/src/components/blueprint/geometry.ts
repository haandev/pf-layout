...
import { BlueprintStore, ViewState } from './store/state';
...
export function naturalFit(state: BlueprintStore): ViewState {
  var view = { ...state.view };
...
export function screenFit(state: BlueprintStore): ViewState {
  var view = { ...state.view };
  if (!state.content.measurement) {
    view.panOffset = p.scale(view.viewSize, 0.5);
    return view;
  ...
  view.panOffset = p.add(
    middle,
    p.scale(
      [-(naturalSize[0] / 2 + state.content.measurement.low[0]), naturalSize[1] / 2 + state.content.measurement.low[1]],
      view.scale
    )
  );
